...
| `export_stride_indip_lab` | `stride_error_indip_lab.csv` | `stride_indip_lab_long_{subject}.csv` | same as `export_stride_lab`, but for a Lab run evaluated against INDIP instead of V3D — no `is_turn`/`is_step` (V3D-only columns) |
...
| `export_stride_outoflab` | `stride_error_indip_outoflab.csv` | `stride_indip_outoflab_long_{subject}.csv` | one device's params for one matched out-of-lab stride, incl. `single_support_s`/`initial_double_support_s`/`terminal_double_support_s`/`double_support_s` — no `is_turn`/`is_step` (V3D-only columns, not part of the INDIP stride schema) |
